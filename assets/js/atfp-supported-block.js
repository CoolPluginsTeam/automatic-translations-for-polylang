class BlockFilterSorter {
  ...
  atfpDataTable(){
    if(this.tableBody){
      const atfpDataTable = new DataTable('#atfp-supported-blocks-table');
      const filterData = atfpDataTable.columns([3]).data().flatten().filter(value => value === 'Supported');
      console.log(atfpDataTable);
      atfpDataTable.row(filterData).draw();
      atfpDataTable.rows(filterData).draw();
      atfpDataTable.data(filterData).draw();
      // atfpDataTable.search(filterData).draw();

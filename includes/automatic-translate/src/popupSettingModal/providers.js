const Providers = ({ Title, Logo, ButtonText, ButtonAction, ButtonDisabled = false, BetaEnabled = false, docs, providerLink, errorMessage = "" }) => {
    return (
        <div className="atfp-providers-wrapper">
            <div className="atfp-providers-links">
            ...
            <div className="atfp-setting-modal-providers">
                <h3>{Title}</h3>
                <img src={Logo} alt={Title} />
                <div className="atfp-setting-modal-providers-button-wrapper">
                    <button onClick={ButtonAction} disabled={ButtonDisabled}>{ButtonText}</button>
                    {BetaEnabled && <span className="atfp-setting-modal-providers-beta">Beta</span>}
                </div>
                {errorMessage && <p className="atfp-error-message">{errorMessage}</p>}
            </div>
        </div>
    );
}

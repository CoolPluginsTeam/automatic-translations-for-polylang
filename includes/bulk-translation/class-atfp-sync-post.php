...
class ATFPP_Sync_Content {
	...
	protected function translate_media( $id ) {
		...
		if ( ! wp_get_post_parent_id( $tr_id ) && 0 < $this->target_post->ID ) {
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- all values are sanitized.
			$wpdb->query( $wpdb->prepare( "UPDATE $wpdb->posts SET post_parent = %d WHERE post_type = 'attachment' AND ID = %d", absint($this->target_post->ID), absint($tr_id) ) );

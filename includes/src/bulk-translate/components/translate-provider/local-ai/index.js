...
class LocalAiTranslate {
    ...
    async createLocalAiTranslator(targetLang, index) {
        ...
        if (this.localAiTranslator.hasOwnProperty('init')) {
            ...
            const translateContentResult = await this.translateContent(0);

            if(translateContentResult && !this.stopTranslation){
    ...
    async translateContent(index) {
        ...
        if(Object.keys(textObject).length === 0){
            this.storeDispatch(unsetPendingPost(this.postId + '_' + this.activeTargetLangs));
            this.storeDispatch(updateTranslatePostInfo({ [this.postId + '_' + this.activeTargetLangs]: { status: 'error', messageClass: 'error', errorMessage: __('No content found to translate', 'automatic-translations-for-polylang'), errorHtml: false } }));
            this.storeDispatch(updateProgressStatus(100 / this.totalPosts));
            return false;
        }

...
const FilterGutenbergContent = async ({content, service, blockParseRules, postId, storeDispatch, filterHtmlContent, sourceLanguage}) => {
    ...
    const filterBlockContent=async (keys, blockContent)=>{
        ...
        if(blockContent.blockName === 'core/image'){
            if(blockContent.attrs && !blockContent.attrs.alt && blockContent.innerHTML && blockContent.innerHTML.trim() !== ''){
                const extractAltText=blockContent.innerHTML.trim().match(/<img[^>]+alt="([^"]*)"/i);
                if(extractAltText && extractAltText[1] && '' !== extractAltText[1]){
                    transltedStrings.push(extractAltText[1]);
                    storeSourceString([...keys, 'attrs', 'alt'].join('_atfp_'), extractAltText[1], extractAltText[1]);
                    blockContent.attrs.alt=extractAltText[1];
                }
            }
        }
        ...
        if(Object.getPrototypeOf(blockRule) === Object.prototype){
            ...
            const runLoopAsyncInner=async(key, index)=>{
                if(typeof blockRule[key] === 'boolean' && true === blockRule[key] && currentBlock && currentBlock[key]){
                    const uniqueKey=[...keys, key].join('_atfp_');
    ...
    const filterBlockAttr=async (keys, block, blockRule)=>{
        ...
        const runLoopAsyncAttr=async(key, index)=>{
            ...
            if(true === activeBlockRule){
                currentKey.push(key);
                const uniqueKey=currentKey.join('_atfp_');
    ...
    const filterContentObject=async (content)=>{
        const runLoopAsync=async(key, index)=>{
            ...
            }else if(null === content[key].blockName && content[key].attrs && content[key].attrs.length < 1 && content[key].innerBlocks && content[key].innerBlocks.length < 1 && content[key].innerContent && content[key].innerContent.length > 0){

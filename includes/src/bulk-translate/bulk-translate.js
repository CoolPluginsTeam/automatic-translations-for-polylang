...
import { updatePendingPosts, unsetPendingPost, updateCompletedPosts, updateTranslatePostInfo, updateCountInfo, updateSourceContent, updateParentPostsInfo, updateTargetContent, updateTargetLanguages, updateBlockParseRules, updateProgressStatus, updateErrorPostsInfo, updateAllowedMetaFields } from './redux-store/features/actions';
...
const initBulkTranslate = async (postKeys = [], nonce, storeDispatch, prefix, updateDestoryHandler) => {
    ...
    const translatePost = async (index) => {
        ...
        if (postContent) {
            const { originalContent: { title, content, excerpt, metaFields }, languages, editorType, sourceLanguage } = postContent;
            ...
            if (!['classic', 'block', 'elementor'].includes(editorType)) {
                for (const lang of languages) {
                    ...
                    storeDispatch(updateTranslatePostInfo({ [postId + '_' + lang]: { status: 'error', messageClass: 'error', errorMessage: __('This post editor type is not supported for translation', 'automatic-translations-for-polylang') } }));
                }
            } else {
                const source = { title: title, content: JSON.parse(JSON.stringify(content)), excerpt: excerpt, metaFields: metaFields };
...
export const updateContent = async ({ source, postId, sourceLang, lang, editorType, createTranslatePostNonce, storeDispatch }) => {
    ...
    const bulkTranslateRouteUrl = atfp_bulk_translate_object.bulkTranslateRouteUrl;
    const nonce = atfp_bulk_translate_object.nonce;
    ...
    if(updateContent.excerpt){
        body.post_excerpt = updateContent.excerpt;
    }
    if(updateContent.metaFields){
        body.post_meta_fields = JSON.stringify(updateContent.metaFields);
    }
    ...
    await fetch(bulkTranslateRouteUrl + `/${postId}/${endPoint}`, {
        ...
        if (data.success && data.data.post_id) {
            ...
            data.data.post_title = '' === data.data.post_title ? __('N/A', 'automatic-translations-for-polylang') : data.data.post_title;
            ...
            if (data.data && data.data.error) {
                ...
                updateData = { status: 'error', messageClass: 'error', errorMessage: __('Post not created. Please try again.', 'automatic-translations-for-polylang'), errorHtml: '<div class="atfp-error-html">' + errorHtml + '</div>' };
            } else if (data.code && data.message) {
                updateData = { status: 'error', messageClass: 'error', errorMessage: __('Post not created. Please try again.', 'automatic-translations-for-polylang'), errorHtml: '<div class="atfp-error-html">' + data.message + '</div>' };
            } else if (!data.success || data.data) {
                updateData = { status: 'error', messageClass: 'error', errorMessage: __('Post not created. Please try again.', 'automatic-translations-for-polylang'), errorHtml: '<div class="atfp-error-html">' + data.data + '</div>' };
            } else if (!data.data.post_id) {
                updateData = { status: 'error', messageClass: 'error', errorMessage: __('Post not created. Please try again.', 'automatic-translations-for-polylang'), errorHtml: '<div class="atfp-error-html">' + data.data + '</div>' };
            } else if (typeof data === 'string') {
                updateData = { status: 'error', messageClass: 'error', errorMessage: __('Post not created. Please try again.', 'automatic-translations-for-polylang'), errorHtml: '<div class="atfp-error-html">' + data + '</div>' };
            }
        }
        ...
        storeDispatch(updateTranslatePostInfo({ [postId + '_' + lang]: { status: 'error', messageClass: 'error', errorMessage: __('Post not created. Please try again.', 'automatic-translations-for-polylang'), errorHtml: '<div class="atfp-error-html">' + errorHtml + '</div>' } }));
    })
}
...
const bulkTranslateEntries = async ({ ids, langs, storeDispatch }) => {
    ...
    const bulkTranslateRouteUrl = atfp_bulk_translate_object.bulkTranslateRouteUrl;
    const bulkTranslatePrivateKey = atfp_bulk_translate_object.bulkTranslatePrivateKey;
    const nonce = atfp_bulk_translate_object.nonce;
    ...
    let postUrl = 'atfp/bulk-translate-entries';
    ...
    if (!untranslatedPostsData) {
        return { success: false, message: __('No posts to translate data undefined', 'automatic-translations-for-polylang') };
    }
    ...
    if (!untranslatedPostsData.data) {
        return { success: false, message: __('No posts to translate untranslated data not found', 'automatic-translations-for-polylang') };
    }

    if (!untranslatedPostsData.data.posts) {
        return { success: false, message: __('No posts to translate untranslated posts data not found', 'automatic-translations-for-polylang') };
    }

    if (!untranslatedPostsData.data.CreateTranslatePostNonce) {
        return { success: false, message: __('No create translate post nonce', 'automatic-translations-for-polylang') };
    }
    ...
    if (postKeys.length > 0) {
        ...
        postKeys.forEach(postId => {
            ...
            if (languages && languages.length > 0) {
                languages.forEach(language => {
                    ...
                    const flagUrl = atfp_bulk_translate_object.languageObject[language].flag;
                    const languageName = atfp_bulk_translate_object.languageObject[language].name;
        ...
        const storeSourceContent = async (index, translatePostsCount) => {
            ...
            const postId = postKeys[index];
            const activeProvider = store.getState().serviceProvider;
            const { title, languages, editor_type, sourceLanguage, excerpt = null, metaFields = null } = posts[postId];
            ...
            if (!sourceLanguage) {
                ...
                const errorInfo = {
                    ...
                    errorMessage: sprintf(
                        __('Set source language for this %s %s before translating.', 'automatic-translations-for-polylang'),
                        titleLink ? '<a href="' + titleLink + '" target="_blank" rel="noopener noreferrer">' + postTitle + '</a>' : postTitle,
                        window?.atfp_bulk_translate_object?.post_label
                    )
                }
            ...
            if (languages && languages.length > 0) {
                ...
                const allowedMetaFields = store.getState().allowedMetaFields;
                const data = { content, editorType: editor_type, service: activeProvider, postId, storeDispatch, sourceLanguage, metaFields, allowedMetaFields, blockParseRules: store.getState().blockParseRules };
...
const getPendingPosts = async (postIds, selectedLanguages, storeDispatch) => {
    const postIdExist = new Array();

    const response = await fetch(atfp_bulk_translate_object.bulkTranslateRouteUrl + '/atfp/pending-posts-ids', {
        method: 'POST',
        body: new URLSearchParams({ ids: JSON.stringify(postIds), lang: JSON.stringify(selectedLanguages), privateKey: atfp_bulk_translate_object.pendingPostsIdsKey }),
        headers: {
            'X-WP-Nonce': atfp_bulk_translate_object.nonce,
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
            'Accept': 'application/json'
        }
    });
    ...
    if (responseData && responseData.success && responseData.data && responseData.data.posts) {

        Object.keys(responseData.data.posts).forEach(postId => {
            const langauges = responseData.data.posts[postId].languages;
            const parentPostTitle = responseData.data.posts[postId].title;

            if (langauges && langauges.length > 0) {
                langauges.forEach(lang => {
                    const flagUrl = atfp_bulk_translate_object.languageObject[lang].flag;
                    const languageName = atfp_bulk_translate_object.languageObject[lang].name;
        ...
        if(responseData.data.blockParseRules){
            storeDispatch(updateBlockParseRules(JSON.parse(responseData.data.blockParseRules)));
        }
...
export { bulkTranslateEntries, initBulkTranslate, getPendingPosts };

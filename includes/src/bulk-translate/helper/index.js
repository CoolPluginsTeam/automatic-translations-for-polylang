...
export const updateTranslateData = ({ provider, sourceLang, targetLang, parentPostId, currentPostId, editorType, updateTranslateDataNonce, extraData = {} }) => {
    ...
    const data = { provider, totalStringCount, totalWordCount, totalCharacterCount, editorType, date, sourceStringCount, sourceWordCount, sourceCharacterCount, sourceLang, targetLang, timeTaken, action: atfp_bulk_translate_object.update_translate_data, atfp_nonce: updateTranslateDataNonce, post_id: currentPostId, ajax_url: atfp_bulk_translate_object.ajax_url, extraData: JSON.stringify(extraData), bulk_translate: true };

    fetch(atfp_bulk_translate_object.ajax_url, {
        method: 'POST',
        headers: {
...
export const translateFieldNameSort=(fieldNames=[])=>{
    ...
    const sortedFieldNames=['title', 'content', 'excerpt', 'metaFields'];

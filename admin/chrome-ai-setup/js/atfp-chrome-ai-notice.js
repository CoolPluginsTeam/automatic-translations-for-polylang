...
class ChromeAINoticeFramework {
    ...
    constructor(options = {}) {
        this.options = Object.assign({
            container: '#atfp-chrome-setup-container',
            dataVar: 'atfpChromeAiNoticeData', // Global localized data object name
        ...
        if ($container.length && typeof ChromeAISetupFramework !== 'undefined') {
            ...
            new ChromeAISetupFramework(this.options.container, {
                ...
                onActionClick: (action) => {
                    if (action === 'switch-yandex') {
                        alert(this.data.texts?.btnAlternativeAlert || 'Please switch the active engine to Yandex in the Dashboard tab.');
                    }
                }
            });
        }
    ...
    async showConfigurationNotice(type) {
        ...
        if (checkResult.hasError) {
            ...
            if (checkResult.errorType !== 'browser' && (type !== 'edge' || browserType !== 'Other')) {
                $(configureBtnSelector).show();
            }
        } else {
...
jQuery(function ($) {
    if (typeof atfpChromeAiNoticeData !== 'undefined') {
        new ChromeAINoticeFramework({
            container: '#atfp-chrome-setup-container',
            dataVar: 'atfpChromeAiNoticeData',
            providerTypes: ['chrome', 'edge']
        });
    }
});
